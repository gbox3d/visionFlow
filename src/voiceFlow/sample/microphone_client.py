...
from common.runtime.audio_codec import encode_wav_bytes
...
async def _send_ping(host: str, port: int, session_id: int) -> None:
    ...
    try:
        ...
        print(f"[AsrIngressClient] ping response: {response.meta}")
    finally:
        writer.close()
    ...
    try:
        ...
        while True:
            ...
            if msg_type == MessageType.ACK:
                print(f"[AsrIngressClient] ack: status={status} meta={response.meta}")
                continue
            if msg_type == MessageType.EVENT:
                print(f"[AsrIngressClient] event: status={status} meta={response.meta}")
                continue
            if msg_type == MessageType.RESULT:
                print(f"[AsrIngressClient] result: {response.meta}")
                return
            print(f"[AsrIngressClient] error: status={status} meta={response.meta}")
            return
    finally:
        writer.close()
        await writer.wait_closed()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Record microphone audio and send it to the NFCP ASR ingress server")
    ...
    print(
        f"[AsrIngressClient] recording {args.duration:.1f}s from microphone "
