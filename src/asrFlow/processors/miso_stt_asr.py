...
BackendName = Literal["ct2", "hf_generate", "hf_pipeline", "qwen_transformers"]
...
class MisoSttAsrProcessor:
    ...
    def _ensure_transcriber(self) -> None:
        ...
        t0 = time.time()
        try:
            if self.backend == "qwen_transformers":
                ...
                from asrFlow.vendors.whisper.transcriber import WhisperTranscriber
            ...
            if not is_ct2_cuda_load_issue:
                if self.model_path is None and _looks_like_download_error(msg):
                    from asrFlow.vendors.whisper.core.config import CT2_CACHE_DIR, HF_CACHE_DIR

                    cache_hint = CT2_CACHE_DIR if self.backend == "ct2" else HF_CACHE_DIR
                    raise RuntimeError(
                        "STT 모델 다운로드 실패(네트워크/SSL). "
                        "모델을 사전 다운로드한 뒤 ASRFLOW_STT_MODEL_PATH에 로컬 모델 경로를 지정하세요. "
